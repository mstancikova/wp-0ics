...
function imax_ibanner_slider () {    
    ...
    for($slideno=1;$slideno<=4;$slideno++){
			$strret = '';
			$slide_title = esc_attr(decodelang(get_theme_mod('itrans_slide'.$slideno.'_title', 'Welcome To i-MAX')));
			$slide_desc = esc_attr(decodelang(get_theme_mod('itrans_slide'.$slideno.'_desc', 'To start setting up i-max go to appearance > customize. Make sure you have installed recommended plugin “TemplatesNext Toolkit” by going appearance > install plugin.')));
			$slide_linktext = esc_attr(decodelang(get_theme_mod('itrans_slide'.$slideno.'_linktext', 'Know More')));
			$slide_linkurl = esc_url(decodelang(get_theme_mod('itrans_slide'.$slideno.'_linkurl', 'http://www.templatesnext.org/icreate/?page_id=541')));
